...
namespace ORB_SLAM2 {
    DynamicExtractor::DynamicExtractor(float confThreshold, float maskThreshold) : confThreshold(confThreshold), maskThreshold(maskThreshold){
        
        string strModelPath = "../Examples/MaskRcnn/pretrained/";
        // define inference parameter                                                                                                        
        string textGraph = strModelPath + "mask_rcnn_inception_v2_coco_2018_01_28.pbtxt";
        string modelWeights = strModelPath +  "mask_rcnn_inception_v2_coco_2018_01_28/frozen_inference_graph.pb";
        string classesFile = strModelPath + "mscoco_labels.names";
        string dynamicClassFile = strModelPath + "dynamic.names";
    ...
    // make binary mask
    Mat DynamicExtractor::extractMask(const Mat &frame, Mat &dynamic_mask) {
        Mat blob;
